...
for m in range(M):
  ...
  rng.choice(X_train, len(X_train), replace=True, axis=1)

  X_d = []
  y_d = []
  randInds = np.random.choice(len(X_train), len(X_train))
  for i in range(0, len(X_train)):
    #print(randIndex)
    X_d.append(X_train[randInds[i]])
    y_d.append(y_train[randInds[i]])
  ...
  clf = tree.DecisionTreeClassifier("entropy", max_features=(len(X_data[0])))
