...
number_of_layers = 2
width_of_layers = 22  # only matters if number of layers > 1
activation = "ReLU" if RELU else "Sigmoid" 
...
def main():
  ...
  res = softmax(logits)
  resV = res.argmax(1)
  sys.stdout = open("NBres2.csv", 'w')
  print("id,digit")
  for i in range(0, len(resV)):
    print(str(i) + "," + str(resV[i]))
...
class LinearLayer:
  ...
  def backward(self, grad):
    ...
    self.grad_bias = np.transpose(grad).sum(axis=1)[:,np.newaxis]
    #print(self.grad_bias[:,np.newaxis].shape)
    #print("grad shape", grad.shape)
    #print("bias shape", self.grad_bias.shape)
    #print("dx shape", (grad@np.transpose(self.weights)).shape)
    return grad @ np.transpose(self.weights)
    
  def step(self, step_size):
    #print("weights shape",self.weights.shape)
    #print("grad weights shape", self.grad_weights.shape)
    self.weights -= step_size*self.grad_weights
    #print("bias shape", self.bias.shape)
    #print("grad_bias shape", self.grad_bias.shape)
    self.bias -= np.transpose(step_size*self.grad_bias)
    #print("all good")
